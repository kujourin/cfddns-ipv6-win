...
如果你希望程序在每次电脑开机（登录系统）时自动运行：
1. 按下 `Win + R` 快捷键打开“运行”窗口。
2. 输入 `shell:startup` 并回车，这会打开当前用户的“启动”文件夹。
3. 为程序（例如 `cfddns-ipv6-x64.exe`）创建一个快捷方式，并将该快捷方式放入这个文件夹中即可。

## 如何退出程序

由于本程序是纯后台静默运行的，没有主界面，你可以通过任务管理器来结束它：
1. 打开任务管理器（快捷键：`Ctrl + Shift + Esc`）。
2. 在“详细信息”或“进程”页面中，找到名为 `cfddns-ipv6-*.exe` 的进程。
3. 右键点击该进程，选择“结束任务”即可关闭程序。
